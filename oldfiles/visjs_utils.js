 ...
 * @param nodePack [{}]
 */
function redrawAll(edgePack, nodePack, container, settingsTemp, changeForeground){
  ...
  if (settings.useImage) {
    for (var m = 0; m < nodesData.length; m++) {
      ...
      nodesData[m].image = '../../images/' + nodesData[m].type + '-icon.png';
...
/**
 * This function is used for synchronizing the controller background swap
 */
function increaseStabilizeTimer(){
  stabilizeTimer++;
}

/**
 * If the background button gets pushed, this function sets the focus on the graph again
 */
function zoomDecrease(){
  network.focusOnNode(nodesData[1].id);
  network.moveTo({
    scale: 0.1
  });
}
 ...
 */
function deactivateNetListeners(){
...
/**
 * Highlights the selected node and the direct neighbours of it.
 * @param selectedItems
 */
...
function restoreOnUnselect(allNodes, allEdges, settings){
  ...
  for (nodeId in allNodes) {
    if (allNodes.hasOwnProperty(nodeId)) {
      if (settings.useImage) {
        allNodes[nodeId].image = '../../images/' + allNodes[nodeId].type + '-icon.png';
      } else {
...
function setColor(allNodes, allEdges, hide, settings){
  ...
  for (var t = 0; t < allNodes.length; t++) {
    ...
    if (allNodes[t].inConnectionList === true) {
      if (settings.useImage) {
        allNodes[t].image = '../../images/' + allNodes[t].type + '-icon.png';
      } else {
      ...
      if (hide) {
        if (settings.useImage) {
          allNodes[t].image = '../../images/' + 'blank.png';
